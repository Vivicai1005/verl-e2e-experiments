...
- `ALL_OFFLOAD=False` — the dummy 2-layer model has tiny GPU footprint
  ...
  run clean.
- `data.dataloader_num_workers=0` — load data in the main process so there are
  no worker children to be OOM-killed (avoids the
  `DataLoader worker ... killed by signal: Killed` traceback at teardown).
